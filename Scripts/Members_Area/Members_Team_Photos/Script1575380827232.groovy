...
GlobalVariable.PAGE = "albums/100647" // Expected, But NOT, as the following 2 lines explain (N/A in this case - correct url returned)
...
for (GlobalVariable.row = 1; GlobalVariable.row <= findTestData('Members_Test_Data').getRowNumbers(); (GlobalVariable.row)++) {
	...
	//-----------------------------------------
	WebUI.callTestCase(findTestCase('Reusable_Test_Sequences/Members_Area/Area_Menu/Non_Dashboard_Navigation/Members_Team_photos'), [:], FailureHandling.CONTINUE_ON_FAILURE)
	
	//-----------------------------------------
	// Confirm Data on Returned Page          |
	//-----------------------------------------
	WebUI.callTestCase(findTestCase('Reusable_Test_Sequences/Members_Area/Validation/Members_Team_photos'), [:], FailureHandling.CONTINUE_ON_FAILURE)
